...
const PartnerLoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await login(email, password, ROLES.PARTNER);
      if (res.success && res.data.role === ROLES.PARTNER) {
        navigate("/partner");
      } else {
        setError(res.message || "Partner login failed");
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center py-12 px-4 bg-slate-100 dark:bg-slate-950">
      ...
      >
        <div className="flex justify-center mb-4">
          <FiBriefcase className="text-brand-600" size={32} />
        </div>
        <div className="flex justify-center mb-6">
          <Logo variant="auth" />
        </div>
        <h1 className="font-display text-2xl font-bold text-center">Partner Login</h1>
        <p className="text-center text-slate-500 text-sm mt-2 mb-6">
          Company portal — post jobs and review student applications
        </p>
        <p className="text-center text-xs text-amber-600 dark:text-amber-400 mb-4">
          Credentials are created by IndLearn admin. Contact admin to reset password.
        </p>
